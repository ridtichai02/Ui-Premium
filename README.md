	--_G.Color = Color3.fromRGB(245, 175, 55)
	...
	function library:CreateWatermark(name) -- getgenv().watermark
		...
		watermark.mainbar.Position = UDim2.new(0, 1,0,15)
	...
	function library:CreateWindow(name, hidebutton)
		local window = { }
		window.name = name or ""
		window.size = UDim2.fromOffset(510, 540)
		...
		function window:CreateTab(name)
			...
			function tab:CreateSector(name, side)
				...
				function sector:AddLabel(text,text2)
					local label = { }
					...
					elseif _G.Setting_table.Show_Ms then
						label.Main.Text = text.." (Ms: "..tostring(text2)..")"
					end
				...
				function sector:AddToggle(text, default, callback, flag)
					...
					function toggle:AddSlider(min, default, max, decimals, callback, flag)
						...
						slider.default = default or slider.min -- string.format("%d",workspace:GetRealPhysicsFPS())
						slider.flag = string.format("%d",flag) or ( (toggle.text or "") .. tostring(#toggle.Items:GetChildren()))
				...
				function sector:AddSlider(text, min, default, max, decimals, callback, flag)
					...
					function slider:Set(value)
						...
						slider.SlideBar:TweenSize(UDim2.fromOffset(percent * slider.Main.AbsoluteSize.X, slider.Main.AbsoluteSize.Y), Enum.EasingDirection.In, Enum.EasingStyle.Sine, 0.05)
						slider.InputLabel.Text = string.format("%d",slider.value)
						pcall(slider.callback, slider.value)
					end
					slider:Set(slider.default)
